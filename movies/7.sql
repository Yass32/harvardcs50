...
SELECT * (SELECT title FROM movies WHERE year = 2010 ORDER BY title)
UNION ALL
SELECT * (SELECT rating FROM ratings WHERE movie_id IN (SELECT id FROM movies WHERE year = 2010) ORDER BY rating DESC);
